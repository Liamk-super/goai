...
export default function ProjectPage({ params }: { params: Promise<{ projectId: string }> }) {
  ...
  return <main className="workspace-main"><header className="workspace-toolbar"><div><p>势能引擎 · 项目操作台</p><strong>{project?.name ?? "加载项目…"}</strong></div><nav><a href="/projects">切换项目</a><a href={`/projects/${projectId}/new-evaluation`}>提交新版本</a>{completedRuns.length > 1 && <a href={`/projects/${projectId}/compare/${completedRuns[0].run_id}`}>版本对比</a>}</nav></header>{error && <p role="alert">{error}</p>}{project && <MomentumWorkbench project={project} runs={runs} team={team} />}</main>;
}
